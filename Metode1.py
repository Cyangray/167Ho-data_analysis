...
import matplotlib
from systlib import GLO_arglist, GLO_hybrid_arglist, upbend_draw_arglist, PDR_to_MeVbarn, SLO_arglist, load_known_gsf, gsf
...
best_gsf.clean_nans()
best_gsf.delete_point([-5,-4,-3,-2,-1])
...
def Sum_function(x, params,is_pdr2 = False):
    if is_pdr2:
...
for i, T in enumerate(Ts):
    ...
    if is_pdr2:
        params_init.extend([E_PDR2, G_PDR2, S_PDR2])
        names.extend(['E_PDR2', 'G_PDR2', 'S_PDR2'])
    ...
    for name,param in zip(names,params_init):
        ...
        if name=='T_GEDR':
            m.limits[name] = (0.85*param,1.1*param)
        #elif name in ['E_PDR1']:#, 'G_PDR1', 'S_PDR1','E_PDR2', 'G_PDR2', 'S_PDR2']:
        #    m.limits[name] = (0.9*param,1.0*param)    
        else:
            #m.limits[name] = (0,None)
            m.limits[name] = (0.5*param,2*param)
    #m.fixed['T_GEDR'] = True
        
    '''
    for name,param in zip(names[13:],params_init[13:]):
        #m.limits[name] = (0,None)
        m.limits[name] = (0.8*param,1.2*param)
    '''
    '''
    m.limits['E_GEDR1'] = (E_GEDR1*0.9,E_GEDR1*1.1)
    m.limits['E_GEDR2'] = (E_GEDR2*0.9,E_GEDR2*1.1)
    m.limits['G_GEDR1'] = (G_GEDR1*0.75,G_GEDR1*1.5)
    
    m.limits['E_PDR1'] = (E_PDR1*0.75,E_PDR1*1.1)
    m.limits['E_SR1'] = (E_SR1*0.75,E_SR1*1.5)
    m.limits['E_PDR1'] = (E_PDR1*0.75,E_PDR1*1.1)    
    m.limits['G_PDR1'] = (G_PDR1*0.75,G_PDR1*1.5)
    m.limits['S_PDR1'] = (S_PDR1*0.75,S_PDR1*1.5)
    
    
    m.limits['G_PDR1'] = (G_PDR1*0.75,G_PDR1*1.5)
    m.limits['S_PDR1'] = (S_PDR1*0.75,S_PDR1*1.5)
    m.limits['G_SR1'] = (G_SR1*0.75,G_SR1*1.5)
    '''
    m.migrad()
    m.hesse()
    ...
    if plot_switch == '%.2f'%(Ts[i]):
        ...
        if is_pdr2:
            ax0.plot(x_values_cont, SLO_arglist(x_values_cont, m.values[13:16]), color = cmap(3/3), linestyle = '-.', label = 'PDR2 fit')
        ax0.plot(GognyM1[:,0], GognyM1[:,1]*const, color = cmap(1/3), linestyle = ':', label = 'Gogny M1')
        ax0.semilogy(x_values_cont, Sum_function(x_values_cont, m.values, is_pdr2), color = cmap(2/3), linestyle = '-', label="Total fit")
        gdr_data.plot(color = cmap(1/4), ax=ax0, linestyle = 'None', marker = '.')
        curr_gsf.plot(color = cmap(4/4), ax=ax0, linestyle = 'None', marker = '.', label=r"$^{167}$Ho, this work")
        ax0.set_xlim([0,10])
        ...
        Tstr = str(int(T*100))
        fig0.tight_layout()
        fig0.savefig('pictures/gsf_fit_metode1_' + Tstr + '.pdf', format = 'pdf')#, bbox_inches='tight')
        fig0.show()
...
for i, T in enumerate(Ts):
    ...
    Tstr = str(int(T*100))
    np.savetxt('minuit2_'+Tstr+'.txt', output, header = header, fmt='%.4f')
